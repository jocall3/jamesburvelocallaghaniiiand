...
// Define types based on the input data structure derived from the project goal description
// Assuming the structure is: id,displayName,appId,createdDateTime,applicationType,accountEnabled,applicationVisibility,assignmentRequired,isAppProxy
type ApplicationType = 'Enterprise Application' | 'Microsoft Application' | 'Managed Identity' | '';
type AccountStatus = 'True' | 'False';
type ApplicationVisibility = 'Visible' | 'Hidden' | '';
type AssignmentRequired = 'True' | 'False';

interface FilterState {
  status: AccountStatus | 'All';
  type: ApplicationType | 'All';
  assignment: AssignmentRequired | 'All';
}
...
interface ApplicationData {
  ...
  isAppProxy: 'True' | 'False';
}

interface FilterBarProps {
  data: ApplicationData[];
  onFilterChange: (filteredData: ApplicationData[]) => void;
}

// Helper function to parse the raw data into structured objects
const parseData = (rawData: string): ApplicationData[] => {
  ...
  dataLines.forEach(line => {
    // Basic CSV splitting, handling quoted fields is complex for this raw format,
    // but based on the sample, fields are mostly simple or enclosed in double quotes if they contain commas.
    // For this specific exercise, we rely on the fixed 9-column structure.
    // We'll use a simple split and assume the structure matches the header for mapping.
    const values = line.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/).map(v => v.trim().replace(/^"|"$/g, ''));
    
    if (values.length === header.length) {
      const [id, displayName, appId, createdDateTime, applicationType, accountEnabled, applicationVisibility, assignmentRequired, isAppProxy] = values;
      ...
      results.push({
        ...
        accountEnabled: accountEnabled as AccountStatus,
        applicationVisibility: applicationVisibility as ApplicationVisibility | '',
        assignmentRequired: assignmentRequired as AssignmentRequired,
        isAppProxy: isAppProxy as 'True' | 'False',
      });
    }
  });
...
// Mock raw data provided in the prompt
const RAW_DATA = `id,displayName,appId,createdDateTime,applicationType,accountEnabled,applicationVisibility,assignmentRequired,isAppProxy
...
687077b3-dde9-477c-bdbf-ffe35e45f233,Azure UpdateCenter,8c420feb-03df-47cc-8a05-55df0cf3064b,2022-04-12T01:26:47Z,Microsoft Application,True,Visible,False,False
...
813ca939-207c-40bb-9b7e-a7cb575ee83d,Quickbooks Other Intuit Service TurboTax,992ddfc7-4a57-44ca-aeb5-ce7846062e10,2022-05-04T08:30:55Z,Enterprise Application,True,Visible,True,False
...
ae593d92-7449-4aad-b774-e2c35c3bab40,Defender for Containers provisioning Azure Policy Addon for Kub,c9455e78-f730-4ba3-a551-cb6a1c389089,2022-04-13T05:11:01Z,Managed Identity,True,Visible,False,False
...
b65d2e06-2631-46d6-963b-23a6d6698695,Defender for Containers provisioning Policy extension for Arc-e,09cf45fc-b312-44eb-a287-e6582ea20ae0,2022-04-13T05:11:00Z,Managed Identity,True,Visible,False,False
...
d5460606-6a8e-4860-adda-dc73283a5065,Azns AAD Webhook,461e8683-5575-4561-ac7f-899cc907d62a,2022-03-28T03:49:01Z,Microsoft Application,True,Visible,False,False
...
df040dd7-d4f0-4f78-a0b3-c2f088079dba,Azure AD Identity Governance - RBAC,3274406e-4e0a-4852-ba4f-d7226630abb7,2022-04-15T14:48:00Z,Microsoft Application,True,Visible,False,False
...
e22935ee-6e70-4bce-9ed1-58e58fc29644,Defender for Containers provisioning Policy extension for Kub,525036f0-a5e3-4a2b-bba7-440d8d71647f,2022-04-13T05:11:00Z,Managed Identity,True,Visible,False,False
...
e819d0ba-62db-4a5d-b156-bc5024daeed3,Azure Service Connector Resource Provider,c4288165-6698-45ba-98a5-48ea7791fed3,2022-04-15T14:43:56Z,Microsoft Application,True,Visible,False,False
...
const ALL_APPLICATIONS = parseData(RAW_DATA);

const getUniqueValues = (data: ApplicationData[], key: keyof ApplicationData, excludeEmpty: boolean = false) => {
  let values = new Set(data.map(item => item[key]));
  if (excludeEmpty) {
    values = new Set([...values].filter(v => v !== '' && v !== null));
  }
...
const FilterBar: React.FC<FilterBarProps> = ({ data, onFilterChange }) => {
  ...
  const availableOptions = useMemo(() => {
    const accountEnabledOptions = getUniqueValues(ALL_APPLICATIONS, 'accountEnabled');
    const applicationTypeOptions = getUniqueValues(ALL_APPLICATIONS, 'applicationType', true);
    const assignmentRequiredOptions = getUniqueValues(ALL_APPLICATIONS, 'assignmentRequired');

    return {
      status: ['All', ...accountEnabledOptions],
      type: ['All', ...applicationTypeOptions],
      assignment: ['All', ...assignmentRequiredOptions],
    };
  }, []);

  const applyFilters = useCallback((currentFilters: FilterState) => {
    const filtered = data.filter(app => {
      // Filter by Status (accountEnabled)
      if (currentFilters.status !== 'All' && app.accountEnabled !== currentFilters.status) {
        return false;
      }
      // Filter by Type (applicationType)
      if (currentFilters.type !== 'All' && app.applicationType !== currentFilters.type) {
        return false;
      }
      // Filter by Assignment (assignmentRequired)
      if (currentFilters.assignment !== 'All' && app.assignmentRequired !== currentFilters.assignment) {
        return false;
      }
      return true;
    });
  ...
  // Initial call to filter the whole dataset on mount/data load (though here data is static for simplicity)
  React.useEffect(() => {
      applyFilters(filters);
  }, [filters, applyFilters]);

  const renderFilter = (name: keyof FilterState, label: string, options: (string | ApplicationType)[]) => (
    <FilterDropdown key={name}>
  ...
  return (
    <FilterBarContainer>
      {renderFilter('status', 'Status', availableOptions.status)}
      {renderFilter('type', 'Type', availableOptions.type as string[])}
      {renderFilter('assignment', 'Assignment Required', availableOptions.assignment)}
    </FilterBarContainer>
  );
};

export default FilterBar;
// --- Styles Definition (For completeness in a real TSX file, usually imported or in styled-components) ---
/*
FilterBar.styles.ts contents mock:
*/
