...
interface EnhancedTableProps {
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Identity) => void;
    order: 'asc' | 'desc';
    orderBy: string;
}
...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler =
        (property: keyof Identity) => (event: React.MouseEvent<unknown>) => {
...
function SortableTable({ data }: SortableTableProps) {
    ...
    const [orderBy, setOrderBy] = React.useState<keyof Identity>('displayName');

    const handleRequestSort =
        (event: React.MouseEvent<unknown>, property: keyof Identity) => {
    ...
    const sortedData = React.useMemo(() => {
        return [...data].sort((a, b) => {
            ...
            if (typeof aValue === 'string' && typeof bValue === 'string') {
                ...
                return orderFactor * (aValue === bValue ? 0 : aValue ? 1 : -1);
            } else {
                return 0; // Handle other types or mixed types
            }
        });
    }, [data, order, orderBy]);
                        ...
                        return (
                            ...
                            >
                                ...
                                <TableCell align="left">
                                    <Chip label={row.accountEnabled.toString()} color={row.accountEnabled ? "success" : "error"} size="small" />
                                </TableCell>
                                <TableCell align="left">{row.applicationVisibility}</TableCell>
                                <TableCell align="left">{row.assignmentRequired.toString()}</TableCell>
                                <TableCell align="left">{row.isAppProxy.toString()}</TableCell>
                                <TableCell align="right">{new Date(row.createdDateTime).toLocaleDateString()}</TableCell>
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const data = `00a67483-ea0d-4bfa-a19b-17ae35e97fe3,ADP,3f18b202-a866-4c1f-b9c0-6598390a2092,2022-05-04T08:14:29Z,Enterprise Application,True,Visible,True,False
...
17c537b0-f0b8-4888-9e9a-346e78542549,Azure Traffic Manager and DNS,2cf9eb86-36b5-49dc-86ae-9a631