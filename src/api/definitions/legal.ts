...
const docusignApi: OpenAPIV3_1.Document = {
    ...
    schemas: {
      ...
      EnvelopeSummary: {
        ...
        properties: {
          ...
          uri: { type: 'string', format: 'uri', readOnly: true, description: 'URI to access the envelope details.', example: '/envelopes/a1b2c3d4-e5f6-7890-1234-567890abcdef' },
          statusDateTime: { type: 'string', format: 'date-time', readOnly: true, description: 'Timestamp of the last status change.', example: '2023-10-27T10:30:00Z' },
        },
        required: ['envelopeId', 'status', 'uri'],
      },
      EnvelopeDocument: {
        type: 'object',
        description: 'Metadata for a document within an envelope.',
        properties: {
          documentId: { type: 'string', description: 'ID of the document within the envelope.', example: '1' },
          name: { type: 'string', description: 'Name of the document.', example: 'NDA.pdf' },
          type: { type: 'string', description: 'Type of document (e.g., "content", "summary").', example: 'content' },
          uri: { type: 'string', format: 'uri', description: 'URI to retrieve the document content.', example: '/envelopes/a1b2c3d4-e5f6-7890-1234-567890abcdef/documents/1' },
          pages: { type: 'integer', description: 'Number of pages in the document.', example: 5 },
          sizeBytes: { type: 'integer', description: 'Size of the document in bytes.', example: 102400 },
        },
        required: ['documentId', 'name', 'type', 'uri'],
      },
    },
    responses: {
      ...commonComponents.responses, // Merge common responses
  ...
  webhooks: {
    envelopeStatusChanged: {
      post: {
        summary: 'Webhook for envelope status changes',
        description: 'Receives notifications when an envelope status changes (e.g., completed, signed), triggering downstream processes.',
        operationId: 'docusignEnvelopeStatusChangedWebhook',
        requestBody: {
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  event: { type: 'string', example: 'envelope-completed', description: 'The type of event that occurred.' },
                  data: { $ref: '#/components/schemas/EnvelopeSummary', description: 'Summary of the envelope that triggered the event.' },
                },
                required: ['event', 'data'],
              },
            },
        ...
        'x-after-script': 'docusign-process-completed-envelope.js',
      },
    },
  },
  externalDocs: {
    description: 'Explore DocuSign eSignature API documentation for more details.',
    url: 'https://developers.docusign.com/docs/esign-rest-api/reference/',
  },
};

/**
 * OpenAPI 3.1.0 definition for the HelloSign API.
 * Provides functionality for sending and managing signature requests and templates,
 * with support for documents from Google Drive.
 */
const hellosignApi: OpenAPIV3_1.Document = {
  openapi: '3.1.0',
  info: {
    title: 'HelloSign API',
    version: 'v3',
    description: 'API for sending and managing signature requests and templates. Supports document content from Google Drive and integrates with project workflows.',
    contact: {
      name: 'HelloSign API Support',
      url: 'https://app.hellosign.com/api/reference',
      email: 'api@hellosign.com',
    },
    license: {
      name: 'HelloSign API Terms',
      url: 'https://www.hellosign.com/legal/terms',
    },
  },
  servers: [
    {
      url: 'https://api.hellosign.com/v3',
      description: 'HelloSign Production API server',
    },
  ],
  security: baseSecurity,
  tags: [
    { name: 'Signature Requests', description: 'Operations related to sending and managing electronic signature requests.' },
    { name: 'Templates', description: 'Operations related to reusable templates for signature requests.' },
  ],
  paths: {
    '/signature_requests/send': {
      post: {
        operationId: 'sendSignatureRequest',
        summary: 'Send a new signature request',
        description: 'Sends a new signature request to one or more signers. Documents can be provided as file uploads or referenced from Google Drive, or a template can be used.',
        ...
        requestBody: {
          ...
          content: {
            'application/json': {
              schema: { $ref: '#/components/schemas/SignatureRequestSend' },
            },
          },
        },
        responses: {
        ...
        'x-workflow-id': 'hellosign-document-signing-workflow',
        'x-pre-script': 'hellosign-prepare-signature-request.js',
        'x-after-script': 'hellosign-log-signature-request.js',
      },
    },
    '/signature_requests/{signatureRequestId}': {
      get: {
        ...
        description: 'Retrieves detailed information for a specific signature request by its ID.',
        tags: ['Signature Requests'],
        parameters: [
          {
    ...
    '/templates': {
      get: {
        ...
        description: 'Retrieves a list of reusable templates that can be used for sending signature requests.',
        tags: ['Templates'],
        responses: {
          '200': {
  ...
  components: {
    schemas: {
      ...
      SignatureRequestSend: {
        type: 'object',
        description: 'Input for sending a signature request. Requires either files or a template ID.',
        properties: {
          title: { type: 'string', description: 'Title of the signature request, visible to signers.', example: 'Client Onboarding Agreement' },
          subject: { type: 'string', description: 'Subject line for the email sent to signers.', example: 'Please sign your onboarding agreement' },
          message: { type: 'string', nullable: true, description: 'Custom message body for the email sent to signers.', example: 'Hi, please find attached the agreement for your review and signature.' },
          signers: {
            type: 'array',
            items: { $ref: '#/components/schemas/SignerInfo' },
            description: 'List of signers for the request.',
            minItems: 1,
            example: [{ name: 'Alice Smith', email_address: 'alice.smith@example.com', order: 1 }]
          },
          files: {
            type: 'array',
            items: { $ref: '#/components/schemas/FileReference' }, // Using common FileReference for Drive integration
            description: 'List of files to be signed. Can be local files (via upload) or Google Drive references. Mutually exclusive with `template_id`.',
            nullable: true,
            example: [{
              fileId: '1sB03_X_Y_Z_A_B_C_D_E_F_G_H_I_J_K',
              fileName: 'Onboarding_Agreement.pdf',
              mimeType: 'application/pdf',
              downloadUrl: 'https://drive.google.com/uc?id=1sB03_X_Y_Z_A_B_C_D_E_F_G_H_I_J_K',
              uploadedAt: '2023-10-27T10:00:00Z',
            }]
          },
          template_id: { type: 'string', nullable: true, description: 'ID of a template to use for the request. Mutually exclusive with `files`.', example: 'a1b2c3d4e5f67890abcdef1234567890' },
          custom_fields: {
            type: 'object',
            additionalProperties: { type: 'string' },
            description: 'Key-value pairs for custom fields if using a template that defines them.',
            example: { 'client_name': 'Alice Smith', 'agreement_date': '2023-10-27' }
          },
        },
        required: ['title', 'signers'],
        oneOf: [ // Must provide either files or a template
          { required: ['files'] },
      ...
      SignerInfo: {
        type: 'object',
        description: 'Information about a signer for a HelloSign request.',
        properties: {
          name: { type: 'string', description: 'Full name of the signer.', example: 'Alice Smith' },
          email_address: { type: 'string', format: 'email', description: 'Email address of the signer.', example: 'alice.smith@example.com' },
          order: { type: 'integer', default: 1, description: 'Order in which the signer receives the request (1-based).', example: 1 },
        },
        required: ['name', 'email_address'],
      },
      SignatureRequest: {
        type: 'object',
        description: 'Details of a sent HelloSign signature request.',
        properties: {
          signature_request_id: { type: 'string', readOnly: true, description: 'Unique identifier for the signature request.', example: 'a1b2c3d4e5f67890abcdef1234567890' },
          title: { type: 'string', description: 'Title of the signature request.', example: 'Client Onboarding Agreement' },
          subject: { type: 'string', description: 'Subject of the email sent to signers.', example: 'Please sign your onboarding agreement' },
          message: { type: 'string', nullable: true, description: 'Custom message sent to signers.', example: 'Hi, please find attached the agreement for your review and signature.' },
          status: { type: 'string', enum: ['awaiting_signature', 'signed', 'declined', 'canceled'], readOnly: true, description: 'Current status of the signature request.', example: 'awaiting_signature' },
          is_complete: { type: 'boolean', readOnly: true, description: 'Indicates if all required signatures have been collected.', example: false },
          has_error: { type: 'boolean', readOnly: true, description: 'Indicates if the signature request encountered an error.', example: false },
          signatures: {
            type: 'array',
            items: {
              type: 'object',
              properties: {
                signature_id: { type: 'string', description: 'Unique ID for this specific signature.', example: 'sig_12345' },
                signer_name: { type: 'string', description: 'Name of the signer.', example: 'Alice Smith' },
                signer_email_address: { type: 'string', format: 'email', description: 'Email address of the signer.', example: 'alice.smith@example.com' },
                status_code: { type: 'string', enum: ['awaiting_signature', 'signed', 'declined', 'canceled'], description: 'Status of this individual signature.', example: 'awaiting_signature' },
                signed_at: { type: 'string', format: 'date-time', nullable: true, description: 'Timestamp when this signature was completed.', example: null },
              },
              required: ['signature_id', 'signer_name', 'signer_email_address', 'status_code'],
            },
            readOnly: true,
            description: 'Details about each signature required for the request.',
          },
          files_url: { type: 'string', format: 'uri', readOnly: true, description: 'URL to download the signed files once the request is complete.', example: 'https://api.hellosign.com/v3/signature_requests/a1b2c3d4e5f67890abcdef1234567890/files' },
        },
        required: ['signature_request_id', 'title', 'status', 'is_complete', 'has_error'],
      },
      Template: {
        type: 'object',
        description: 'A reusable HelloSign template for consistent signature requests.',
        properties: {
          template_id: { type: 'string', readOnly: true, description: 'Unique identifier for the template.', example: 'temp_a1b2c3d4e5f67890abcdef1234567890' },
          title: { type: 'string', description: 'Title of the template.', example: 'Standard NDA Template' },
          message: { type: 'string', nullable: true, description: 'Default message for requests using this template.', example: 'This is our standard Non-Disclosure Agreement.' },
          signer_roles: {
            type: 'array',
            items: {
              type: 'object',
              properties: {
                name: { type: 'string', description: 'Role name (e.g., "Client", "Witness").', example: 'Client' },
                order: { type: 'integer', description: 'Order of this role in the signing process.', example: 1 },
              },
              required: ['name', 'order'],
            },
            description: 'Defined roles for signers in this template.',
          },
          custom_fields: {
            type: 'array',
            items: {
              type: 'object',
              properties: {
                name: { type: 'string', description: 'Name of the custom field.', example: 'Effective Date' },
                type: { type: 'string', enum: ['text', 'checkbox', 'date'], description: 'Type of the custom field.', example: 'date' },
                required: { type: 'boolean', description: 'Indicates if this custom field is required.', example: true },
              },
              required: ['name', 'type', 'required'],
            },
            description: 'Custom fields defined within the template.',
          },
        },
  ...
  webhooks: {
    signatureRequestSigned: {
      post: {
        summary: 'Webhook for signature request signed event',
        description: 'Receives notifications when a signature request is fully signed, enabling automated archiving to Google Drive or other post-signing actions.',
        operationId: 'hellosignSignatureRequestSignedWebhook',
        requestBody: {
          content: {
            'application/json': {
              schema: {
                type: 'object',
                properties: {
                  event: { type: 'string', example: 'signature_request_signed', description: 'The type of event that occurred.' },
                  signature_request: { $ref: '#/components/schemas/SignatureRequest', description: 'The signature request object that was signed.' },
                },
                required: ['event', 'signature_request'],
              },
            },
          },
        },
        ...
        'x-after-script': 'hellosign-archive-signed-document-to-drive.js',
      },
    },
  },
  externalDocs: {
    description: 'Learn more about the HelloSign API and its capabilities.',
    url: 'https://app.hellosign.com/api/reference',
  },
};

/**
 * Defines the structure for a collection of legal API OpenAPI documents.
 */
export type LegalApiDefinitions = {
  clio: OpenAPIV3_1.Document;
  docusign: OpenAPIV3_1.Document;
  hellosign: OpenAPIV3_1.Document;
};

/**
 * Exports a constant containing OpenAPI 3.1.0 definitions for various legal APIs.
 * Each definition includes schemas, paths, security, webhooks, and custom extensions
 * to support project-specific features like Google Drive integration and workflow scripts.
 */
export const legalApiDefinitions: LegalApiDefinitions = {
  clio: clioApi,
