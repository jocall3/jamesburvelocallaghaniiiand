const rawApplicationsCsv = `id,displayName,appId,createdDateTime,applicationType,accountEnabled,applicationVisibility,assignmentRequired,isAppProxy
00a67483-ea0d-4bfa-a19b-17ae35c97fe3,ADP,3f18b202-a866-4c1f-b9c0-6598390a2092,2022-05-04T08:14:29Z,Enterprise Application,True,Visible,True,False
...
// In-memory store for applications, simulating a backend database
let applications: Application[] = [];

function parseCsv(csvString: string): Application[] {
  ...
  return dataLines.map(line => {
    ...
    headers.forEach((header, index) => {
      ...
      // Clean up header names to match interface properties (e.g., remove quotes)
      const cleanedHeader = header.replace(/"/g, '');

      // Type conversion for boolean fields
      if (value === 'True') value = true;
      else if (value === 'False') value = false;
      // Convert empty strings to null/undefined or keep as empty string based on needs.
      // For this mock, keeping as empty string is fine.

      (app as any)[cleanedHeader] = value;
    });
    return app as Application;
  });
}

// Initialize applications data from CSV
applications = parseCsv(rawApplicationsCsv);

/**
 * Simulates fetching a list of applications.
 * @returns A promise that resolves with an array of Application objects.
 */
export const getApplications = async (): Promise<Application[]> => {
  // Simulate network delay
  await new Promise(resolve => setTimeout(resolve, 300));
  return [...applications]; // Return a copy to prevent direct mutation of internal state
};

/**
 * Simulates revoking access for a specific application.
 * In this mock, it sets `accountEnabled` to `false` and `assignmentRequired` to `true`.
 * @param appId The ID of the application for which to revoke access.
 * @returns A promise that resolves with a success boolean and a message.
 */
export const revokeAccess = async (appId: string): Promise<{ success: boolean; message: string }> => {
  // Simulate network delay
  ...
  if (appIndex !== -1) {
    // In a real scenario, revocation might involve more complex state changes or actual deletion.
    // Here, we're marking it as disabled and requiring assignment for clarity in the mock.
    applications[appIndex] = {
      ...applications[appIndex],
      accountEnabled: false,
      assignmentRequired: true, // Assuming revocation might imply future re-assignment is required
    };
    return { success: true, message: `Access revoked for application with ID: ${appId}` };
  } else {
    return { success: false, message: `Application with ID: ${appId} not found.` };
  }
};

/**
 * Simulates initiating an access review for a specific application.
 * In this mock, it simply logs the action and returns a success status.
 * @param appId The ID of the application for which to start an access review.
 * @returns A promise that resolves with a success boolean and a message.
 */
export const startAccessReview = async (appId: string): Promise<{ success: boolean; message: string }> => {
  // Simulate network delay
  ...
  if (app) {
    console.log(`Mock: Initiating access review for application "${app.displayName}" (App ID: ${appId}).`);
    // In a real implementation, this would trigger an actual access review process.
    return { success: true, message: `Access review initiated for application with ID: ${appId}` };
  } else {
    return { success: false, message: `Application with ID: ${appId} not found for access review.` };
  }
};
